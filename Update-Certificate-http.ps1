...
.PARAMETER notIIS
.EXAMPLE
    C:\PS> Update-Certificate-http -alias "www-mydomain" -domain "www.mydomain.com" -websiteName "My Website"
    This command creates a certificate for www.mydomain.com which is hosted by the web site named 'My Website'
.EXAMPLE
    C:\PS> Update-Certificate-http -alias "www-mydomain" -domain "www.mydomain.com" -websiteName "My Website" -domains @{"alias1"="my.domain.com";"alias2"="other.domain.com"}
...
#>
Function Update-Certificate-Http
{
    param
    (
    ...
    {
        "The web site '$WebSiteName' is not started.  It will not be possible to complete the challenge"
        return
    }
    ...
    $domains.GetEnumerator() | ForEach-Object {
        ...
        if ( ! ( (Get-ACMEVault).Identifiers.Count -and (Get-ACMEIdentifier | Where-Object { $_.Alias -eq "$alias" }).Count ) )
        {
            "Create identifier for '$domain' using alias '$alias'"
            $result = New-ACMEIdentifier -Dns $domain -Alias $alias
        }
        else
        {
            "The identifier '$alias' already exists"
            $result = Get-ACMEIdentifier $alias
        }
        ...
        if ( ! ($result.Challenges | Where-Object { $_.Type -eq 'http-01' -and $_.SubmitDate }).Count )
        {
            # No?  Has the request been submitted?
            if ( ! ($result.Challenges | Where-Object { $_.Type -eq 'http-01' -and $_.HandlerName -eq 'manual' }).Count )
            {
                "Issue challenge"
            ...
            $content = ""
            $path = ""
            $result.Challenges | Where-Object { $_.Type -eq 'http-01' } | ForEach-Object { $content = $_.Challenge.FileContent; $path = $_.Challenge.FilePath }
            if ( ! $content )
            {
            ...
            "Submit challenge"
            $result = Submit-ACMEChallenge $alias -ChallengeType http-01
    ...
    $domains.GetEnumerator() | ForEach-Object {
        ...
        if ( $identifiers[$alias].Challenges | Where-Object { $_.Type -eq 'http-01' -and $_.Status -eq 'valid' } ) { "$alias is valid"; return }
        if ( $identifiers[$alias].Challenges | Where-Object { $_.Type -eq 'http-01' -and $_.Status -eq 'invalid' } ) { "$alias is invalid"; return }

        $result = Update-ACMEIdentifier $alias -ChallengeType http-01
        $identifiers[$alias] = $result
    ...
    $identifiers.GetEnumerator() | ForEach-Object {
        ...
        if ( ($_.Value.Challenges | Where-Object { $_.Type -eq 'http-01' -and $_.Status -eq 'invalid' } ) )
        {
            $abort = $true;
    ...
    $maxAttempts = 10
    $attempts = 0
    while ( ($identifiers | Where-Object { $_.Challenges | Where-Object { $_.Type -eq 'http-01' -and $_.Status -ne 'valid' } } ) )
    {
        $attempts++
        ...
        $domains.GetEnumerator() | ForEach-Object {
            ...
            if ( ( $result.Challenges | Where-Object { $_.Type -eq 'http-01' -and $_.Status -eq 'pending' } ) )
            {
                $result = Update-ACMEIdentifier $alias -ChallengeType http-01
                $identifiers[$alias] = $result
                if ( ($result.Challenges | Where-Object { $_.Type -eq 'http-01' -and $_.Status -eq 'invalid' } ) )
                {
    ...
    try
    {
        $result = Get-ACMECertificate $certalias
    ...
    catch
    {
        ...
        $new = New-ACMECertificate $mainalias -Generate -Alias $certalias -CsrDetails $csrDetails -AlternativeIdentifierRefs ([array]($domains.GetEnumerator() | Where-Object { $_.Key -ne $mainalias } | ForEach-Object { $_.Key } ))

        try
        {
            $error.Clear()
            $result = Submit-ACMECertificate $certalias
        }
        catch
        {
    ...
    $result = Update-ACMECertificate $certalias
    $attempts = 0
    ...
    { 
        $result = Get-ACMECertificate $certalias -ExportPkcs12 $filepath -Overwrite
    }
    else
    {
